...
const blogSchema = new mongoose.Schema(
  {
    ...
    slug: { // NEW FIELD: Slug for friendly URLs
      type: String,
      required: true,
      unique: true,
      lowercase: true,
...
// Keeping this route for backward compatibility if needed, or if fetching by _id is still desired for internal use.
...
app.post(
  ...
  async (req, res) => {
    try {
      const { title, content, category, subcategory, author, status, slug: providedSlug } =
      ...
      if (err.code === 11000 && err.keyPattern && err.keyPattern.slug) {
        return res.status(409).json({ message: "A blog with a similar title/slug already exists. Please choose a unique title or provide a custom slug.", error: err.message });
      }
